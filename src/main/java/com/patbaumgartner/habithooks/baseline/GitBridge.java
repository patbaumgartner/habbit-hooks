 ...
 *
 * <p>
 * All methods return {@link Optional#empty()} or {@code false} when the git command
 * fails, so callers do not need to handle exceptions.
 */
public final class GitBridge {
    ...
    public Optional<String> lastCommitHash(String filePath) {
        try {
            ...
            LOGGER.error("Could not get last commit hash for {}: {}", filePath, e.getMessage(), e);
    ...
    /**
     * Returns {@code true} when the working tree has staged or unstaged changes to the
     * given file.
     * @param filePath workspace-relative path to the file
    ...
    public boolean isDirty(String filePath) {
        try {
            ...
            LOGGER.error("Could not check dirty status for {}: {}", filePath, e.getMessage(), e);
            if (e instanceof InterruptedException) {
                Thread.currentThread().interrupt();
            }
            return false;
     ...
     * @return newline-separated list of changed file paths, or empty string on failure
     */
    public String changedFilesSinceBranch(String branchBase) {
        try {
            ...
            LOGGER.error("Could not list changed files: {}", e.getMessage(), e);
            if (e instanceof InterruptedException) {
                Thread.currentThread().interrupt();
            }
            return "";
    ...
    public String changedFilesInLastN(int n) {
        try {
            return exec("git", "diff", "--name-only", "HEAD~" + n + "...HEAD");
        }
        catch (IOException | InterruptedException e) {
            LOGGER.error("Could not list changed files: {}", e.getMessage(), e);
            if (e instanceof InterruptedException) {
    ...
    /**
     * Returns the list of files changed since the given commit hash.
     * @param commitHash a git commit SHA or ref (e.g. {@code "abc123"})
     * @return newline-separated list of changed file paths, or empty string on failure
     */
    public String changedFilesSinceCommit(String commitHash) {
        try {
            return exec("git", "diff", "--name-only", commitHash + "...HEAD");
        }
        catch (IOException | InterruptedException e) {
            LOGGER.error("Could not list changed files since {}: {}", commitHash, e.getMessage(), e);
            if (e instanceof InterruptedException) {
                Thread.currentThread().interrupt();
            }
            return "";
