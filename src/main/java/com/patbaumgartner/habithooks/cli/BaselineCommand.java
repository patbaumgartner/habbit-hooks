 ...
 */
@Command(name = "baseline", mixinStandardHelpOptions = true, description = "Manage the violation baseline",
        subcommands = { BaselineCommand.SnoozeSubcommand.class, BaselineCommand.PruneSubcommand.class,
...
public final class BaselineCommand implements Runnable {
    ...
    @Command(name = "snooze", mixinStandardHelpOptions = true, description = "Add current violations to the baseline")
    static final class SnoozeSubcommand implements Runnable {
        ...
        public void run() {
            ...
            System.out.println("Baseline updated. " + violations.size() + " violation(s) snoozed.");
        }

    }

    /** Removes baseline entries for files that no longer exist. */
    @Command(name = "prune", mixinStandardHelpOptions = true, description = "Remove baseline entries for deleted files")
    ...
    @Command(name = "status", mixinStandardHelpOptions = true, description = "Summarise the current baseline")
