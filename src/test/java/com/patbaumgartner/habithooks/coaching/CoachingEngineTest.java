...
class CoachingEngineTest {
    ...
    private final CoachingEngine engine = new CoachingEngine(new PromptLoader(Path.of("/nonexistent")));
    ...
    void groupsViolationsByRule() {
        List<Violation> violations = List.of(new Violation("checkstyle:MethodLength", "A.java", 1, "Too long"),
        ...
        CoachingGroup methodLength = groups.stream()
            .filter(g -> "checkstyle:MethodLength".equals(g.ruleId()))
            .findFirst()
            .orElseThrow();
    ...
    void coachedGroupsAppearBeforeUncoached() {
        List<Violation> violations = List.of(new Violation("custom:UnknownRule", "X.java", 1, "unknown"),
    ...
    void titlesAreAssignedToKnownRules() {
        List<Violation> violations = List.of(new Violation("pmd:GodClass", "Big.java", 1, "God class"));
    ...
    void unknownRuleUsesRuleIdAsTitle() {
        List<Violation> violations = List.of(new Violation("custom:MyRule", "Foo.java", 1, "msg"));
        AnalysisResult result = new AnalysisResult(violations, 1);
        List<CoachingGroup> groups = engine.coach(result);
